  ...
  return (
    <div className="restaurant-shell">
      ...
      <div className="panel glass-card">
        {orders.length === 0 ? <div className="empty-state">No orders found for this restaurant yet.</div> : <table className="table"><thead><tr><th>Order</th><th>Customer</th><th>Status</th><th>Payment</th><th>Total</th></tr></thead><tbody>{orders.map((order) => <tr key={order._id} className="table-row"><td>{order._id?.slice(-6).toUpperCase()}</td><td>{order.customerName || order.userId?.name || 'Guest'}</td><td>{order.status}</td><td>{order.paymentStatus}</td><td>KES {Number(order.totalPrice || 0).toLocaleString()}</td></tr>)}</tbody></table>}
      </div>
    </div>
  );
};
